...
set fp [open "valxx.txt" w+]
set fp1 [open "valyy.txt" w+]
for {set i 0} {$i < $val(nn)} {incr i} {
$node_($i) set X_ [expr rand()*$val(x)]
puts $fp [expr rand()*$val(x)]
$node_($i) set Y_ [expr rand()*$val(y)]
puts $fp1 [expr rand()*$val(y)]
$node_($i) set Z_ 0
$node_($i) color "green"
}
close $fp1
...
# -------------------------------------- #	fp2	
set fp2 [open "/home/user01/Documents/anu/input.txt" r]
set filed [read $fp2]
...
# --------------------------------------# 	fp1
set fp1 [open "/home/user01/Documents/anu/mali.txt" r]
...
if {$length1 !=0} {
set id $line
#$ns at 3.2 "[$node_($id) set ragent_]hacker"
